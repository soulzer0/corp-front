<template>
  <div id="cart_checkout" class="fw">
    <table id="cartDisplay">
      <thead>
        <tr>
          <th></th>
          <th>Info. do Produto</th>
          <th>Quantidade</th>
          <th>Preço</th>
        </tr>
      </thead>
      <tbody>
        ...
        >
          ...
          <td>
            <div class="cql">
              <button class="qtd_btn menos" @click="subtraiQtd(index)">
                -
              </button>
              {{ item.quantidade }}
...
export default {
  ...
  computed: {
    ...
    valorTotal() {
      return (
        parseFloat(this.freteValor) + parseFloat(this.totalProdutos)
      ).toFixed(2);
    },
  },
  methods: {
    ...
    subtraiQtd: function (index) {
      if (this.cartItens[index].quantidade == 1) {
        this.$toasted.show("Este produto será removido do seu carrinho", {
          action: [
            {
              text: "Remover",
              onClick: (e, toastObject) => {
                ...
                this.$toasted.show("Produto removido do carrinho!");
              },
            },
            {
              text: "Cancelar",
    ...
    checkCheckout: function () {
      ...
      if (!this.logado) {
        this.errors.push(
          "Você precisar estar logado para realizar uma compra!"
        );
      }
    },
